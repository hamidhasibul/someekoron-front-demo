      ...
      <div class="row">
        ...
        <div class="sidebar-container-right">
          <div class="row border-bottom py-3 px-2 align-items-center">
            <div class="col-lg-12">
              <p class="mb-0 text-color fs-20">Total Resources</p>
            </div>
          </div>
          ...
          <div class="container my-3">
            <div class="row justify-content-center">
              <div class="col-lg-10 border shadow-sm py-4 print_view" id="printID">
                ...
                <p class="fs-14 text-center mb-0">Total Resources</p>
                ...
                <table class="table align-middle">
                  <thead>
                    <tr class="thead-color border">
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600 ps-4">#Resource ID</th>

                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600">Resource Name</th>

                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600">Resource Directory</th>
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600">Quantity</th>
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600">Total Cost</th>
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600">Entry Date</th>
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600">Vendor</th>
                    </tr>
                  </thead>
                  <tbody class="border-0">
                    <tr class="shadowItem" v-for="item in filteredData" :key="item.id">
                      ...
                      <td scope="col" class="border-0 fs-12">
                        {{ item.pname ? item.pname : "General" }}
                      </td>
                      <td scope="col" class="border-0 fs-12">
                        {{ parseFloat(item.transfer_amount).toFixed(2) }} {{ item.ptype }}
                      </td>
                      <td scope="col" class="border-0 fs-12">
                        {{ (item.transfer_amount * item.unitCost).toFixed(2) }}
                      </td>
                      <td scope="col" class="border-0 fs-12">
                    ...
                    <tr class="thead-color border">
                      ...
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600"></th>
...
export default {
  ...
  methods: {
    ...
    getJournalData() {
      fetch(`${process.env.VUE_APP_SERVER_URL}/getResouceData`, {
        method: "POST",
      })
        .then((res) => res.json())
        .then((data) => {
          this.totalResourceData = data.message.filter((item) => {
            return item.transfer_project !== "";
          });

          console.log(this.totalResourceData);
        })
  ...
  computed: {
    ...mapState(["orgInfo"]),
    filteredData() {
      const { startDate, endDate, totalResourceData } = this;

      return totalResourceData.filter(({ edate }) => {
        return (startDate <= edate && endDate >= edate) || startDate === "";
      });
    },

    totalQty() {
      return this.filteredData.reduce((total, item) => total + +item.transfer_amount, 0);
    },
    totalCost() {
      return this.filteredData.reduce((total, item) => total + +item.transfer_amount * +item.unitCost, 0);
    },
