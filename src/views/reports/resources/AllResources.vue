                ...
                <table class="table align-middle">
                  ...
                  <tbody class="border-0">
                    <tr class="shadowItem" v-for="item in filteredData" :key="item.id">
                      ...
                      <td scope="col" class="border-0 fs-12">{{ item.qty }} {{ item.ptype }}</td>
                      <td scope="col" class="border-0 fs-12">
                        {{ item.tcost }}
                      </td>
                      <td scope="col" class="border-0 fs-12">
...
export default {
  ...
  methods: {
    ...
    getJournalData() {
      fetch(`${process.env.VUE_APP_SERVER_URL}/getResouceData`, {
        ...
        .then((res) => res.json())
        .then((data) => {
          this.totalResourceData = data.message;
  ...
  computed: {
    ...
    totalQty() {
      return this.filteredData.reduce((total, item) => total + +item.qty, 0);
    },
    totalCost() {
      return this.filteredData.reduce((total, item) => total + +item.tcost, 0);
    },
