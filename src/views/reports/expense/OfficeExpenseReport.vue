      ...
      <div class="row">
        ...
        <div class="sidebar-container-right">
          <div class="row border-bottom py-3 px-2 align-items-center">
            <div class="col-lg-12">
              <p class="mb-0 text-color fs-20">Office Expense Report</p>
            </div>
          </div>
          ...
          <div class="container my-3">
            <div class="row justify-content-center">
              <div class="col-lg-10 border shadow-sm py-4 print_view" id="printID">
                ...
                <p class="fs-14 text-center mb-0">Office Expenses</p>
                  ...
                  <tbody class="border-0">
                    <tr class="shadowItem" v-for="item in filteredData" :key="item.id">
                      ...
                      <td scope="col" class="border-0 fs-12 font-weight-600">
                        <span class="d-inline-block text-truncate">{{ item.ex_des }}</span>
                      </td>
...
export default {
  ...
  methods: {
    ...
    getExpenseData() {
      fetch(`${process.env.VUE_APP_SERVER_URL}/getExpenseData`, {
        ...
        .then((res) => res.json())
        .then((data) => {
          this.officeExpenses = data.message.filter((item) => {
            return item.exp_type === "Office Expense";
          });
  ...
  computed: {
    ...mapState(["orgInfo"]),
    filteredData() {
      const { startDate, endDate, officeExpenses } = this;

      return officeExpenses.filter(({ cdate }) => {
