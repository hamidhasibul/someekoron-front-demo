      ...
      <div class="row">
        ...
        <div class="sidebar-container-right">
          <div class="row border-bottom py-3 px-2 align-items-center">
            <div class="col-lg-12">
              <p class="mb-0 text-color fs-20">Total Outgoing Transaction</p>
            </div>
          </div>
          ...
          <div class="container my-3">
            <div class="row justify-content-center">
              <div class="col-lg-10 border shadow-sm py-4 print_view" id="printID">
                ...
                <p class="fs-14 text-center mb-0">Total Outgoing Transactions</p>
                ...
                <table class="table align-middle">
                  <thead>
                    <tr class="thead-color border">
                      ...
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600 text-end pe-5">Amount</th>
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600 text-end pe-5">Amount (+VAT)</th>
                    </tr>
                  </thead>
                  <tbody class="border-0">
                    <tr class="shadowItem" v-for="item in filteredData" :key="item.id">
                      ...
                      <td scope="col" class="border-0 fs-12 font-weight-600">
                        <span class="d-inline-block text-truncate" v-if="item.ex_des !== ''">{{ item.ex_des }}</span>

                        <span v-if="item.project_name !== ''" class="d-inline-block text-truncate">{{
                          item.project_name
                        }}</span>
                      </td>

                      <td scope="col" class="border-0 fs-12 text-end pe-5">
                    ...
                    <tr class="thead-color border">
                      ...
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600 text-end pe-5">
                        Total: {{ parseFloat(totalAmount).toFixed(2) }}
                      </th>
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600 text-end pe-5">
                        Total: {{ parseFloat(totalAmountVat).toFixed(2) }}
                      </th>
                    </tr>
                  </tbody>
                </table>
...
export default {
  ...
  methods: {
    ...
    getJournalData() {
      fetch(`${process.env.VUE_APP_SERVER_URL}/getOutgoingData`, {
        method: "POST",
      })
        .then((res) => res.json())
        .then((data) => {
          this.totalOutgoingData = data.message;
        })
  ...
  computed: {
    ...mapState(["orgInfo"]),
    filteredData() {
      const { startDate, endDate, totalOutgoingData } = this;

      return totalOutgoingData.filter(({ cdate }) => {
    ...
    totalAmountVat() {
      this.totalAmountCount = this.filteredData.reduce((total, item) => total + +item.t_amount, 0);
