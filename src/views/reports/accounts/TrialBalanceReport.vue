      ...
      <div class="row">
        ...
        <div class="sidebar-container-right">
          <div class="row border-bottom py-3 px-2 align-items-center">
            <div class="col-lg-12">
              <p class="mb-0 text-color fs-20">Trial Balance</p>
            </div>
          </div>
          ...
          <div class="container my-3">
            <div class="row justify-content-center">
              <div class="col-lg-10 border shadow-sm py-4 print_view" id="printID">
                ...
                <p class="fs-14 text-center mb-0">Trial Balance</p>
                ...
                <table class="table align-middle">
                  <thead>
                    <tr class="thead-color border">
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600 ps-4">Accounts Head</th>

                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600">Debit</th>
                      <th scope="col" class="border-0 fs-13 text-muted font-weight-600">Credit</th>
                    </tr>
                  </thead>
                  <tbody class="border-0">
                    <tr class="shadowItem" v-for="item in calculatedLedgerData" :key="item.id">
                      <template v-if="item.entries && item.entries.length !== 0"
                        ...
                        <td scope="col" class="border-0 fs-12 font-weight-600">
                          <span class="d-inline-block text-truncate">{{ item.debit }}</span>
                        </td>
                    ...
                    <tr class="shadowItem">
                      <th scope="col" class="border-0 fs-12 ps-4">Total</th>
                      <th scope="col" class="border-0 fs-12 font-weight-600">
                        <span class="d-inline-block text-truncate">{{ totalDebit }}</span>
                      </th>

                      <th scope="col" class="border-0 fs-12">
                        {{ totalCredit }}
                      </th>
                    </tr>
                  </tbody>
...
export default {
  ...
  computed: {
    ...
    totalDebit() {
      this.totalDebitNew = this.calculatedLedgerData.reduce((total, item) => total + +item.debit, 0);
      return this.calculatedLedgerData.reduce((total, item) => total + +item.debit, 0);
    },
    totalCredit() {
      this.totalCreditNew = this.calculatedLedgerData.reduce((total, item) => total + +item.credit, 0);
      return this.calculatedLedgerData.reduce((total, item) => total + +item.credit, 0);
    },
  ...
  updated() {
    console.log(this.calculatedLedgerData);
  },
};
</script>
