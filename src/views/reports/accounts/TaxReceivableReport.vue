      ...
      <div class="row">
        ...
        <div class="sidebar-container-right">
          <div class="row border-bottom py-3 px-2 align-items-center">
            <div class="col-lg-12">
              <p class="mb-0 text-color fs-20">Tax Receivable Report</p>
            </div>
          </div>
          ...
          <div class="container my-3">
            <div class="row justify-content-center">
              <div class="col-lg-10 border shadow-sm py-4 print_view" id="printID">
                ...
                <p class="fs-14 text-center mb-0">Tax Receivable Report</p>
                ...
                <table class="table align-middle">
                  <thead>
                    <tr class="thead-color border">
                      ...
                      <th scope="col" class="w-25 fs-13 text-muted font-weight-600 text-end pe-5">Vat Ammount</th>
                      <th scope="col" class="w-25 fs-13 text-muted font-weight-600 text-end pe-5">Tax Ammount</th>
                    </tr>
                  </thead>
                  <tbody class="">
                    <tr class="shadowItem" v-for="item in filteredData" :key="item.id">
                      ...
                      <td v-if="item.t_tax > 0 || item.tax_amount > 0" scope="col" class="fs-12">
                        <span class="d-inline-block text-truncate">
                          {{ item.project_name }}, {{ item.org_name }}, ({{ item.preceipt_id }})
                        </span>
                      </td>
                    ...
                    <tr class="shadowItem">
                      ...
                      <th scope="col" class="border-0 fs-12 text-end pe-5">Total: {{ totalVatAmount }}</th>
                      <th scope="col" class="border-0 fs-12 text-end pe-5">Total: {{ totalTaxAmount }}</th>
                    </tr>
                  </tbody>
                </table>
              </div>
...
export default {
  ...
  methods: {
    ...
    getTaxReceivableData() {
      fetch(`${process.env.VUE_APP_SERVER_URL}/getPReceiptData`, {
        method: "POST",
      })
        .then((res) => res.json())
        .then((data) => {
          this.taxReceivableData = data.message;
  ...
  computed: {
    ...
    filteredData() {
      const { startDate, endDate, taxReceivableData } = this;
      console.log(this.taxReceivableData);
      return taxReceivableData.filter(({ cdate }) => {
